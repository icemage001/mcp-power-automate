...
import {
  ...
  previewFlowUpdateInputSchema,
  revertLastUpdateInputSchema,
  targetInputSchema,
...
export const commandDefinitions: CommandDefinition[] = [
  ...
  createCommand({
    description: 'Preview an edit for an explicit environment and flow, returning baseline and proposal hashes plus high-risk change reasons.',
    handler: (input) => previewFlowUpdate(input),
    inputSchema: previewFlowUpdateInputSchema,
    name: 'preview_flow_update',
    risk: 'read',
  }),
  createCommand({
    description: 'Validate a flow definition for an explicit environment and flow using the flow service API when available.',
  ...
  createCommand({
    description: 'List immutable pre-update backups for the explicitly identified flow.',
    handler: (input) => listFlowBackups(input),
    inputSchema: targetInputSchema,
    name: 'get_flow_backups',
    risk: 'read',
  }),
  createCommand({
    description: 'Apply the exact previewed definition to its explicit target. Requires matching baseline and proposal hashes; high-risk changes also require explicit confirmation.',
  ...
  createCommand({
    description: 'Revert the last recorded update for an explicit target after checking its current flow hash; high-risk reverts require confirmation.',
    handler: (input) => revertLastUpdate(input),
    inputSchema: revertLastUpdateInputSchema,
  ...
  createCommand({
    description: 'Run a request trigger for an explicit target. This can cause business side effects.',
